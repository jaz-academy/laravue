        ...
        >
          <div>
            <div class="d-flex align-center">
              ...
              <RouterLink :to="`/front/bookmark?name=${abbreviateName(props.studentName, 10, 1).toLowerCase()}`" class="d-flex flex-column">
            ...
            <RouterLink :to="`/front/bookmark?theme=${abbreviateName(props.theme, 30, 4).toLowerCase()}`" class="d-flex flex-column">
              <p class="text-body-1 text-primary">
                {{ props.subject +': '+ props.theme }}
              </p>
            </RouterLink>
            <p class="text-body-2 d-inline">
              <RouterLink :to="`/front/bookmark?id=${taskId}`" class="text-warning">
            ...
            <div class="d-flex align-center mt-4">
              ...
              <div class="d-flex flex-column">
                <RouterLink :to="`/front/bookmark?theme=${abbreviateName(mentor.name, 30, 4).toLowerCase()}`" class="d-flex flex-column">
                  <h6 class="text-base text-primary ms-1">
                    {{ mentor.name }}
                  </h6>
                </RouterLink>
                <div 
                  class="text-sm text-medium-emphasis cursor-pointer"
                >
                  <template v-if="participant && participant.role > 1">
                    <div @click="showModalRating = true" class="d-flex flex-column">
                      <VRating
                        readonly
                        v-model="rating"
                        half-increments
                  ...
                  <template v-else>
                    <RouterLink 
                      :to="`/front/bookmark?stars=${rating}`" 
            ...
            <div v-if="showModalRating" class="modal-overlay">
                  ...
                  <VRow>
                    ...
                    <VCol cols="12">
                      <AppSelect
                        ...
                        :items="['Yes', 'No']"
                        @update:modelValue="handleRatingCollumn"
                      />
                    </VCol>
                    
                    <VCol v-if="ratingCol" cols="12" class="text-center">
                      <VRating
                        v-model="rating"
                        half-increments
                        hover
